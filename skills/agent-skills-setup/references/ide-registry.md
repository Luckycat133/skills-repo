...
### cline
- **detect**: `~/.cline/`
- **mcp** (global/shared): current Config docs publish `~/.cline/data/settings/cline_mcp_settings.json`; the MCP guide also documents `~/.cline/mcp.json` for the CLI. Both use root key `mcpServers` · JSON. The mapper uses the Config-layout path for a fresh install, preserves an existing alternative, honors `CLINE_MCP_PATH`/`CLINE_DATA_DIR`, and stops for manual selection if both files exist. Do not substitute a guessed VS Code `globalStorage` path.
