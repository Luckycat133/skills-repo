...
> **Full IDE Registry**: Read `skills/agent-skills-setup/references/ide-registry.md` for detailed per-IDE paths of all migration objects.
...
Root key and format differences cause the most common migration errors. The full per-IDE registry — paths for **all** migration objects (not just MCP) across 40 IDEs — lives in `skills/agent-skills-setup/references/ide-registry.md`. Load only what you need, e.g.:
...
| ZCode | `mcp.servers` | JSON | `~/.zcode/cli/config.json` / `.zcode/config.json` | Native MCP is nested; Settings → MCP Servers → Import can import Claude/Codex/OpenCode/.agents sources; project Skills use Settings → Skills → Import with Copy/Symlink + Global/Project target, not a guessed `.zcode/skills` path |
| Void Editor | `mcpServers` | JSON | global `~/.void-editor/mcp.json` | Official repository is archived/deprecated; legacy custom store remains narrow support. Local `command`/`args`/`env` is the safe automatic shape, URL-only remote is limited, headers/auth and inherited VS Code `.vscode/mcp.json` (`servers`) remain manual; no Agent Skills directory |
...
| Amazon Q Developer | `mcpServers` | JSON | `~/.aws/amazonq/default.json` / `.amazonq/default.json`; legacy `mcp.json` is preserved when already present | AWS also documents `agents/default.json` for another Q surface; if it is the only existing file, choose the active product manually. Review `useLegacyMcpJson`, permissions, OAuth, and CLI agent scope. |
...
CONVERT_MCP(source_config, source_ide, target_ide):
  ...
  2. For each server entry:
     ...
     d. Handle special fields:
        ...
        - Claude Desktop: legacy local JSON is a narrow automatic target only at the documented macOS/native-Windows path; modern local MCP uses Settings → Extensions/.mcpb and remote MCP uses Settings → Connectors. Linux and MSIX-specific paths remain manual.
...
`skills/agent-skills-setup/scripts/auto-configure-openclaw-skills.sh` is the installer for **OpenClaw**, a
...
| Roo Code | Explicit project MCP is `.roo/mcp.json` with root key `mcpServers`; global MCP is extension-settings/UI managed with no stable official path. Project `mcp` requires `--scope project`; global remains manual. Skills use `~/.roo/skills` and `.roo/skills`; `.roorules` is the single-file rules target and `.roo/commands/*.md` is exposed as prompts with semantic review. Roo Code is not Cline, and its MCP config is not VS Code's `servers` schema. Archived 2026-05; migrate to Kilo Code. |
