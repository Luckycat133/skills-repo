...
- 发布文档和 changelog 已更新。
- The `--version` must equal the current `version:` field in `skills/agent-skills-setup/SKILL.md` (currently `0.6.5`); update it on every release.
- `--version` 必须与 `skills/agent-skills-setup/SKILL.md` 中当前的 `version:` 一致（当前为 `0.6.5`），每次发布时同步更新。
...
bash skills/agent-skills-setup/scripts/prepare-clawhub-release.sh \
    ...
    --changelog "Fix VS Code profile safety and MCP conflict semantics; add OpenCode V2, deterministic JSON evidence, complete test discovery, and leaner guidance."
```
